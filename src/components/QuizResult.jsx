...
const QuizResult = ({ score, nextQuizLink, buttonName = 'Next Challenge' }) => {
    ...
    return (
        <div className='flex items-center justify-center'>
            <div className="w-full sm:w-[80%] text-white p-4 rounded-md mt-10 mb-5 relative">
                ...
                <div className="absolute top-0 left-0 right-0 bottom-[20%] sm:bottom-36 flex flex-col items-center justify-center">
                    <h1 className="text-2xl sm:text-5xl font-bold p-2 sm:p-10">{title}</h1>
                    <p className="w-2/3 sm:w-1/3 text-center text-xl sm:text-3xl p-2">{desc}</p>
                </div>
                <div style={{ '--text-color': bgColor }} className="parent-class">
                    <Link to={nextQuizLink} className="absolute left-1/2 transform -translate-x-1/2 bottom-0 sm:bottom-20 btn-light rounded-xl px-6 py-4 font-bold text-[var(--text-color)] hover:bg-light-yellow transition-transform duration-200 hover:scale-110">
                        {buttonName}
                    </Link>
                </div>
            </div>
